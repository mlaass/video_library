...
class FaceLibrary:
    ...
    def build_from_photos(self, progress_callback=None):
        ...
        person_dirs = [d for d in os.listdir(self.photos_dir) if os.path.isdir(os.path.join(self.photos_dir, d))]
        ...
        for i, person_name in enumerate(person_dirs):
            ...
            photo_files = [f for f in os.listdir(person_dir) if f.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))]
...
class FaceRecognizer:
    """Main face recognition processor for videos with SQLite database"""
    ...
    def __init__(self, library_dir: str = None):
        ...
        self.library_dir = library_dir
        self.faces_dir = os.path.join(library_dir, "faces")
        self.db_path = os.path.join(library_dir, "face_library.db")
    ...
    def extract_faces_from_segment(
        self, video_path: str, video_filename: str, start_time: float, duration: float = 10, tolerance: float = 0.6
    ) -> List[Dict]:
        """Detailed face recognition for a video segment (Pass 2) - saves to database"""
        faces_found = []
        ...
        try:
            ...
            while cap.get(cv2.CAP_PROP_POS_MSEC) < end_time * 1000:
                ...
                for encoding, location in zip(face_encodings, face_locations):
                    ...
                    face_hash = self.db.find_matching_identity(encoding, tolerance)

                    if face_hash is None:
                        # New face - create identity
                        face_hash = self.db.add_identity(encoding, label=None)

                    # Get label (or None if unlabeled)
                    identity = self.db.get_identity(face_hash)
                    ...
                    # Generate unique filename: <hash>_<uuid>.jpg
                    unique_id = str(uuid.uuid4())[:8]
                    image_filename = f"{face_hash}_{unique_id}.jpg"
    ...
    def process_video(self, video_path: str, task_id: str = None, progress_callback=None) -> Dict:
        """Process entire video with two-pass face recognition and SQLite storage"""
        ...
        try:
            ...
            for i, start_time in enumerate(face_segments):
                if progress_callback:
                    progress = 35 + int((i / len(face_segments)) * 60) if face_segments else 95
                    progress_callback(f"Pass 2: Analyzing segment {i+1}/{len(face_segments)}", progress)

                faces = self.extract_faces_from_segment(video_path, video_filename, start_time)

                for face_data in faces:
                    timestamp = face_data["timestamp"]
                    face_hash = face_data["face_hash"]
                    label = face_data["label"]

                    if timestamp not in results["faces_by_timestamp"]:
                        results["faces_by_timestamp"][timestamp] = []

                    results["faces_by_timestamp"][timestamp].append({"hash": face_hash, "label": label})
                    results["face_hashes"].add(face_hash)
                    if label:
                        results["labeled_people"].add(label)
                    results["total_faces"] += 1

            # Convert sets to lists for JSON serialization
            results["face_hashes"] = list(results["face_hashes"])
            results["labeled_people"] = list(results["labeled_people"])
            results["processing_time"] = time.time() - start_processing

            if progress_callback:
                progress_callback(
                    f"Complete! Found {results['total_faces']} faces, "
                    f"{len(results['face_hashes'])} unique identities, "
                    f"{len(results['labeled_people'])} labeled people",
                    100,
                )
...
def process_video_face_recognition(video_path: str, task_id: str, processing_status: Dict):
    ...
    try:
        ...
        def update_progress(message, progress):
            processing_status[task_id] = {"status": "processing", "progress": progress, "details": message}

        # Process video
        results = face_recognizer.process_video(video_path, task_id, update_progress)

        if "error" in results:
            processing_status[task_id] = {"status": "error", "message": results["error"]}
            return
        ...
        processing_status[task_id] = {"status": "error", "message": f"Face recognition error: {str(e)}"}
